...
def evaluate(env, agent, video, num_episodes, L, step, config):
    all_ep_rewards = []
...
class KoopmanActor(nn.Module):
    ...
    def __init__(
        ...
        self.config = config
        # XL: for now hardcode it for cart-pole swingup task. Change it for other tasks
    ...
    def forward(
        ...
        # goal_obs = self.encoder(self.goal_obs.repeat(obs.shape[0], 1, 1, 1), detach=detach_encoder)
...
class CurlSacKoopmanAgent(CurlSacAgent):
    ...
    def update_actor_and_alpha(self, obs, next_obs, action, L, step):
        ...
        # XL: maybe add more loss terms for embedlqr
    ...
    def update_kpm(self, obs, next_obs, action, L, step, use_ls=False):
        ...
        # update critic's encoder (should we update actor's encoder? maybe not because actor's encoder is tied to critic's)
    ...
    def update(self, replay_buffer, L, step):
        ...
        if step % self.koopman_update_freq == 0 and self.encoder_type == 'pixel':
